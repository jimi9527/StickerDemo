...
public class GlcameraActivity extends Activity implements MyCamera.IPreView{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        nv21 = new byte[myCamera.PREVIEW_WIDTH * myCamera.PREVIEW_HEIGHT*2];



    }

     void initGLSurfaceview(){
...
//			int config = 0; //default config
        ...
        killed = false;
        final byte[] tmp = new byte[myCamera.PREVIEW_WIDTH * myCamera.PREVIEW_HEIGHT * 2];
        thread = new Thread(){
            @Override
            public void run() {
                super.run();
...
//					STMobile106[] faces = tracker.track(tmp, dir,PREVIEW_WIDTH,PREVIEW_HEIGHT);
                    ...
                    STMobileFaceAction[] faceActions = tracker.trackFaceAction(tmp, dir, myCamera.PREVIEW_WIDTH,
                            myCamera.PREVIEW_HEIGHT);
